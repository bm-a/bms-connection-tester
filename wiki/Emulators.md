# Emulators — test everything without the battery (v1.2 results)

## Layer 1 — native tests + soak (Termux, always)
`sh run_tests.sh` (g++ + local Unity fallback) and `pio test -e native`:
checksum (7), logic (8), parser (13), stress (4) = **32/32 PASS**,
plus `tools/soak_sim.cpp` 8-day run (`691040 polls / 691040 replies`,
millis-wrap crossed, green-on-resume) — PASS. Protocol core untouched by v1.2.
...
## Layer 3 — Wokwi browser sim (functional proof, incl. RGB)
...
(green while polling, red ~2 s after stopping the meter).

## Layer 4 — QEMU-S3 (boot only, known gap)
Built per `bm-a/esp32s3-qemu-arm64` (source build in proot, sanitized PATH,
RDID/SFDP patches). v1.2 8 MB image merged with `--fill-flash-size 8MB`:
...
Auto-skips without hardware. Bench expectation for v1.2: meter shows
≈ 52 V / 100 %, discretes + RGB agree.
